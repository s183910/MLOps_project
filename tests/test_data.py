...
def load_dataset(path, n, shape_of_record):
    ...
    assert len(dataset) == n, f"Dataset did not have the correct number of samples: {len(dataset)} != {n}"
    ...
    assert images.shape[0] == shape_of_record[0] and images.shape[1] == shape_of_record[1], image_shape_error
    assert labels.shape[0] == shape_of_record[0], label_shape_error

@pytest.mark.skipif(not os.path.exists('data/raw/sign_mnist_train.csv'), reason="Data files not found (training)")
def test_load_training_dataset():
    path = os.path.join(PATH_DATA, "raw/sign_mnist_train.csv")
    N_train = 27455
    load_dataset(path, N_train, [1, 784])

@pytest.mark.skipif(not os.path.exists('data/raw/sign_mnist_test.csv'), reason="Data files not found (testing)")
